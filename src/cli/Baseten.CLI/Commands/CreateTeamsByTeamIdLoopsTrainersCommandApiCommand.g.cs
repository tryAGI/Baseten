...
internal static partial class CreateTeamsByTeamIdLoopsTrainersCommandApiCommand
{
...
Creates a trainer-only Loops run in the given session, without a sampler. To sample from it, create a sampler with POST /v1/loops/samplers and pass this run's ID as run_id to pair the two. List and read the trainer through the /v1/loops/runs endpoints.");
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                        ...
                        var teamId = parseResult.GetRequiredValue(TeamId);                        var sessionId = parseResult.GetRequiredValue(CreateLoopsRunRequestV1OptionSetOptions.SessionId);
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.CreateTeamsByTeamIdLoopsTrainersAsync(
                                    teamId: teamId,
                                    sessionId: sessionId,
                                    baseModel: baseModel,
                                    name: name,
